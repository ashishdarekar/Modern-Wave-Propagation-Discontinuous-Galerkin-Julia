...
"""
 struct GlobalMatrices
     ...
     function GlobalMatrices(basis::Basis, filter::Filter, dimensions)
        ...
        project_flux_to_face = Dict(f => face_projection_matrix(basis, f) for f in faces)
        project_dofs_from_face = Dict(f => transpose(face_projection_matrix(basis, f)) for f in faces)
        ...
        filter_matrix = evaluate_filter_matrix(filter, basis)
         ...
         project_flux_to_face, project_dofs_from_face, basissize_1d, quadweights_nd,
